...
public:
  UniformAlloc(int start, int end)
      : start(start), end(end), bound(end), tend(end) {}
...
static inline UniformAlloc &getAlloc(int type, const DVLEData *dvle) {
  ...
  default:
  case UTYPE_FVEC:
    return unifAlloc[x].fvecAlloc;
  case UTYPE_IVEC:
    return unifAlloc[x].ivecAlloc;
  case UTYPE_BOOL:
    return unifAlloc[x].boolAlloc;
  }
}
  ...
  for (; str; str = nextStr, curLine += nextLineIncr) {
    ...
    for (;;) {
      ...
      *colonPos = 0;
      ...
      std::pair<labelTableIter, bool> ret = g_labels.insert(
          std::pair<std::string, size_t>(labelName, BUF.size()));
      if (!ret.second)
...
int FixupLabelRelocations() {
  for (relocTableIter it = g_labelRelocTable.begin();
       it != g_labelRelocTable.end(); ++it) {
    relocation &r = *it;
...
int RelocateProduct() {
  for (relocTableIter it = g_procRelocTable.begin();
       it != g_procRelocTable.end(); ++it) {
    relocation &r = *it;
    u32 &inst = BUF[r.first];
  ...
  if (g_totalDvleCount == 0)
    return throwError(
        "no DVLEs can be generated from the given input file(s)\n");
...
static char *nextArg() { return trim_whitespace(mystrtok(NULL, ",")); }

static char *nextArgCParen() { return trim_whitespace(mystrtok(NULL, "(")); }

static char *nextArgSpc() { return trim_whitespace(mystrtok_spc(NULL)); }
...
#define NEXT_ARG_CPAREN(_varName)                                              \
  char *_varName;                                                              \
  do {                                                                         \
...
#define NEXT_ARG_OPT(_varName, _opt)                                           \
  char *_varName;                                                              \
  do {                                                                         \
    _varName = nextArg();                                                      \
    if (!_varName)                                                             \
...
#define DEC_COMMAND(name, fun) {#name, cmd_##fun, MAESTRO_##name, -1}
...
#define DEC_DIRECTIVE(name) {#name, dir_##name, 0, 0}

#define DEC_DIRECTIVE2(name, fun, opc) {#name, dir_##fun, opc, 0}
...
static inline int ensure_valid_src_narrow(int reg, const char *name,
                                          int srcId) {
  if (reg < 0x00 || reg >= 0x20)
    return throwError("invalid source%d register: %s\n", srcId, name);
  return 0;
...
#define ARG_TARGET(_argName) safe_call(ensureTarget(_argName))
...
static int maskFromSwizzling(int sw, bool reverse = true) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (bitid > prevbitid)
      fprintf(stderr,
              "%s:%d: warning: arbitrary swizzling has no effect for "
              "destination mask\n",
              curFile, curLine);
    prevbitid = bitid;
    if (reverse)
...
static void optimizeOpdesc(int &mask, int opcode, int opdesc) {
  ...
  case MAESTRO_MOVA:
    if (!(opdesc & BIT(3 - COMP_X)))
      unused1 |= SWIZZLE_COMP(0, 3);
    if (!(opdesc & BIT(3 - COMP_Y)))
      unused1 |= SWIZZLE_COMP(1, 3);
  case MAESTRO_CMP:
    unused1 |= SWIZZLE_COMP(2, 3) | SWIZZLE_COMP(3, 3);
    break;
  }

  mask &= ~OPDESC_MAKE(0, OPSRC_MAKE(0, unused1), OPSRC_MAKE(0, unused2),
                       OPSRC_MAKE(0, unused3));
}
...
static inline int convertIdxRegName(const char *reg) {
  if (stricmp(reg, "a0") == 0 || stricmp(reg, "a0.x") == 0)
    return 1;
  if (stricmp(reg, "a1") == 0 || stricmp(reg, "a0.y") == 0)
    return 2;
  if (stricmp(reg, "a2") == 0 || stricmp(reg, "lcnt") == 0 ||
      stricmp(reg, "aL") == 0)
    return 3;
  return 0;
}
...
static inline int parseCondOp(const char *name) {
  if (stricmp(name, "eq") == 0)
    return COND_EQ;
  if (stricmp(name, "ne") == 0)
    return COND_NE;
  if (stricmp(name, "lt") == 0)
    return COND_LT;
  if (stricmp(name, "le") == 0)
    return COND_LE;
  if (stricmp(name, "gt") == 0)
    return COND_GT;
  if (stricmp(name, "ge") == 0)
    return COND_GE;
  return -1;
}
...
static int parseReg(char *pos, int &outReg, int &outSw, int *idxType = NULL) {
  ...
  if (idxType && *idxType && (outReg < 0x20 || outReg >= 0x80))
    return throwError(
        "index register not allowed with this kind of register\n");
  outReg += regOffset;
  return 0;
}
...
static inline bool isBadInputRegCombination(int a, int b, int c) {
  return isBadInputRegCombination(a, b) || isBadInputRegCombination(b, c) ||
         isBadInputRegCombination(c, a);
}
...
DEF_COMMAND(format1) {
  ...
  if (isBadInputRegCombination(rSrc1, rSrc2))
    return throwError(
        "source operands must be different input registers (v0..v15)\n");

  int opdesc = 0;
  safe_call(findOrAddOpdesc(
      opcode, opdesc,
      OPDESC_MAKE(maskFromSwizzling(rDestSw), rSrc1Sw, rSrc2Sw, 0),
      OPDESC_MASK_D12));

#ifdef DEBUG
  printf("%s:%02X d%02X, d%02X, d%02X (0x%X)\n", cmdName, opcode, rDest, rSrc1,
         rSrc2, opdesc);
#endif
  if (!inverted)
    BUF.push_back(FMT_OPCODE(opcode) | opdesc | (rSrc2 << 7) | (rSrc1 << 12) |
                  (rSrc1Idx << 19) | (rDest << 21));
  else
    BUF.push_back(FMT_OPCODE(opcodei) | opdesc | (rSrc2 << 7) | (rSrc1 << 14) |
                  (rSrc2Idx << 19) | (rDest << 21));
...
DEF_COMMAND(format1u) {
  ...
  int opdesc = 0;
  safe_call(findOrAddOpdesc(
      opcode, opdesc, OPDESC_MAKE(maskFromSwizzling(rDestSw), rSrc1Sw, 0, 0),
      OPDESC_MASK_D1));
...
DEF_COMMAND(format1c) {
  ...
  int opdesc = 0;
  safe_call(findOrAddOpdesc(opcode, opdesc, OPDESC_MAKE(0, rSrc1Sw, rSrc2Sw, 0),
                            OPDESC_MASK_12));

#ifdef DEBUG
  printf("%s:%02X d%02X, %d, %d, d%02X (0x%X)\n", cmdName, opcode, rSrc1, cmpx,
         cmpy, rSrc2, opdesc);
#endif
  BUF.push_back(FMT_OPCODE(opcode) | opdesc | (rSrc2 << 7) | (rSrc1 << 12) |
                (rSrc1Idx << 19) | (cmpy << 21) | (cmpx << 24));
...
DEF_COMMAND(format5) {
  ...
  bool inverted = opcodei >= 0 && rSrc2 < 0x20 &&
                  (rSrc3 >= 0x20 || (rSrc3Idx && !rSrc2Idx));
  ...
  if (isBadInputRegCombination(rSrc1, rSrc2, rSrc3))
    return throwError(
        "source registers must be different input registers (v0..v15)\n");

  int opdesc = 0;
  safe_call(findOrAddOpdesc(
      opcode, opdesc,
      OPDESC_MAKE(maskFromSwizzling(rDestSw), rSrc1Sw, rSrc2Sw, rSrc3Sw),
      OPDESC_MASK_D123));
...
#ifdef DEBUG
  printf("%s:%02X d%02X, d%02X, d%02X, d%02X (0x%X)\n", cmdName, opcode, rDest,
         rSrc1, rSrc2, rSrc3, opdesc);
#endif
  if (!inverted)
    BUF.push_back(FMT_OPCODE(opcode) | opdesc | (rSrc3 << 5) | (rSrc2 << 10) |
                  (rSrc1 << 17) | (rSrc2Idx << 22) | (rDest << 24));
  else
    BUF.push_back(FMT_OPCODE(opcodei) | opdesc | (rSrc3 << 5) | (rSrc2 << 12) |
                  (rSrc1 << 17) | (rSrc3Idx << 22) | (rDest << 24));
...
DEF_COMMAND(formatmova) {
  ...
  int mask;
  if (stricmp(targetReg, "a0") == 0 || stricmp(targetReg, "a0.x") == 0)
    mask = BIT(3);
  else if (stricmp(targetReg, "a1") == 0 || stricmp(targetReg, "a0.y") == 0)
    mask = BIT(2);
  else if (stricmp(targetReg, "a01") == 0 || stricmp(targetReg, "a0.xy") == 0)
    mask = BIT(3) | BIT(2);
  else
    return throwError("invalid destination register for mova: %s\n", targetReg);

  ARG_TO_SRC1_REG2(rSrc1, src1Name);

  int opdesc = 0;
  safe_call(findOrAddOpdesc(opcode, opdesc, OPDESC_MAKE(mask, rSrc1Sw, 0, 0),
                            OPDESC_MASK_D1));
...
#ifdef DEBUG
  printf("%s:%02X vtx%d, %s, %s\n", cmdName, opcode, vtxId,
         isPrim ? "true" : "false", isInv ? "true" : "false");
#endif
  BUF.push_back(FMT_OPCODE(opcode) | ((u32)isInv << 22) | ((u32)isPrim << 23) |
                (vtxId << 24));
...
DEF_COMMAND(format2) {
  ...
  case MAESTRO_JMPC: {
    ...
    relocTableType &rt =
        opcode == MAESTRO_CALLC ? g_procRelocTable : g_labelRelocTable;
...
DEF_COMMAND(format3) {
  ...
  case MAESTRO_JMPU: {
    ...
    relocTableType &rt =
        opcode == MAESTRO_CALLU ? g_procRelocTable : g_labelRelocTable;
    rt.push_back(std::make_pair(BUF.size(), targetName));
...
DEF_DIRECTIVE(else) {
  ...
  if (lastWasEnd) {
    ...
    if (lastOpcode == MAESTRO_JMPC || lastOpcode == MAESTRO_JMPU ||
        lastOpcode == MAESTRO_CALL || lastOpcode == MAESTRO_CALLC ||
        lastOpcode == MAESTRO_CALLU || (p - elem.pos) < 2)
      insertPaddingNop();
  }
...
DEF_DIRECTIVE(end) {
  ...
  if (elem.type != SE_ARRAY && lastWasEnd) {
  ...
  else if ((elem.type == SE_PROC || elem.type == SE_FOR ||
            elem.type == SE_IF) &&
           BUF.size() > 0) {
    u32 p = BUF.size();
    u32 lastOpcode = BUF[p - 1] >> 26;
    if (lastOpcode == MAESTRO_JMPC || lastOpcode == MAESTRO_JMPU ||
        lastOpcode == MAESTRO_CALL || lastOpcode == MAESTRO_CALLC ||
        lastOpcode == MAESTRO_CALLU ||
        (elem.type == SE_FOR &&
         (lastOpcode == MAESTRO_BREAK || lastOpcode == MAESTRO_BREAKC)) ||
        (elem.type != SE_ARRAY &&
         (p - elem.pos) < (elem.type != SE_PROC ? 2 : 1)))
      insertPaddingNop();
  }
...
#endif
    g_procTable.insert(std::pair<std::string, procedure>(
        elem.strExtra, procedure(elem.pos, size)));
    break;
  }
  ...
  case SE_ARRAY: {
    ...
    if (g_constArraySize >= 0)
      for (; size < g_constArraySize; size++) {
        Constant c;
        memset(&c, 0, sizeof(c));
        c.type = UTYPE_FVEC;
    ...
    g_aliases.insert(std::pair<std::string, int>(
        g_constArrayName, uniformPos | (DEFAULT_OPSRC << 8)));
...
DEF_DIRECTIVE(alias) {
  ...
  g_aliases.insert(
      std::pair<std::string, int>(aliasName, rAlias | (rAliasSw << 8)));
  return 0;
}
...
DEF_DIRECTIVE(uniform) {
  ...
  for (;;) {
    ...
    for (i = 0; useSharedSpace && i < g_uniformCount; i++) {
      ...
      if (uniform.name == argText) {
        ...
        if (uniform.size != uSize)
          return throwError(
              "uniform '%s' previously declared as having size %d\n", argText,
              uniform.size);
    ...
    if (useSharedSpace)
      g_uniformTable[g_uniformCount++].init(argText, uniformPos, uSize,
                                            dirParam);
    ...
    if (*argText != '_') {
      ...
      dvle->uniformTable[dvle->uniformCount++].init(argText, uniformPos, uSize,
                                                    dirParam);
      dvle->symbolSize += strlen(argText) + 1;
    }

    g_aliases.insert(std::pair<std::string, int>(
        argText, uniformPos | (DEFAULT_OPSRC << 8)));

#ifdef DEBUG
    printf("uniform %s[%d] @ d%02X:d%02X\n", argText, uSize, uniformPos,
           uniformPos + uSize - 1);
#endif
  }
  return 0;
...
DEF_DIRECTIVE(const) {
  ...
  *parenPos = 0;
  ...
  g_aliases.insert(
      std::pair<std::string, int>(constName, ct.regId | (DEFAULT_OPSRC << 8)));

#ifdef DEBUG
  if (dirParam == UTYPE_FVEC)
    printf("constant %s(%f, %f, %f, %f) @ d%02X\n", constName, ct.fparam[0],
           ct.fparam[1], ct.fparam[2], ct.fparam[3], ct.regId);
  else if (dirParam == UTYPE_IVEC)
    printf("constant %s(%u, %u, %u, %u) @ d%02X\n", constName, ct.iparam[0],
           ct.iparam[1], ct.iparam[2], ct.iparam[3], ct.regId);
#endif
  return 0;
};
...
static int parseBool(bool &out, const char *text) {
  ...
  if (stricmp(text, "false") == 0 || stricmp(text, "off") == 0 ||
      stricmp(text, "0") == 0) {
    out = false;
    return 0;
  }
...
DEF_DIRECTIVE(in) {
  ...
  g_aliases.insert(
      std::pair<std::string, int>(inName, oid | (DEFAULT_OPSRC << 8)));
  return 0;
}
...
DEF_DIRECTIVE(out) {
  ...
  if (oid < 0)
    ...
    return throwError(
        "this output collides with another one previously defined\n");
  ...
  if (outName)
    g_aliases.insert(
        std::pair<std::string, int>(outName, oid | (DEFAULT_OPSRC << 8)));
...
DEF_DIRECTIVE(gsh) {
  ...
  if (dvle->constantCount || dvle->uniformCount || dvle->outputMask)
    return throwError(".gsh must be used before any constant, uniform or "
                      "output is declared\n");
  ...
  case GSHTYPE_FIXED: {
    ...
    if (arrayStart >= firstFreeReg)
      return throwError(
          "specified location overlaps uniform allocation pool: %s\n",
          arrayStartText);
...
int ProcessCommand(const char *cmd) {
  ...
  if (*cmd == '.') {
    ...
    if (!GetDvleData()->isGeoShader && g_outputBuf.size() > MAX_VSH_SIZE)
      return throwError("instruction outside vertex shader code memory (max %d "
                        "instructions, currently %d)\n",
