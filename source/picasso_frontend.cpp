...
int usage(const char *prog) {
  fprintf(stderr,
          ...
          "Options:\n"
          "  -o, --out=<file>        Specifies the name of the SHBIN file to "
          "generate\n"
          "  -h, --header=<file>     Specifies the name of the header file to "
          "generate\n"
          "  -n, --no-nop            Disables the automatic insertion of "
          "padding NOPs\n"
...
int main(int argc, char *argv[]) {
  ...
  int opt, optidx = 0;
  while ((opt = getopt_long(argc, argv, "o:h:?nv", long_options, &optidx)) !=
         -1) {
    switch (opt) {
    case 'o':
      shbinFile = optarg;
  ...
  if (hFile) {
    ...
    const char *prefix =
        g_dvleTable.front().isGeoShader
            ? "GSH"
            : "VSH"; // WARNING: HORRIBLE HACK - PLEASE FIX!!!!!!!
    for (int i = 0; i < g_uniformCount; i++) {
      Uniform &u = g_uniformTable[i];
      const char *name = u.name.c_str();
      if (*name == '_')
        continue; // Hidden uniform
