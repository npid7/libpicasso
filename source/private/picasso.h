...
#define OPDESC_MAKE(out, src1, src2, src3)                                     \
  ((out) | ((src1) << 4) | ((src2) << (4 + 9)) | ((src3) << (4 + 9 * 2)))
#define FMT_OPCODE(n) ((n) << 26)
#define OUTPUT_MAKE(i, reg, mask) ((i) | ((reg) << 16) | ((u64)(mask) << 32))

#define DEFAULT_SWIZZLE                                                        \
  (SWIZZLE_COMP(0, COMP_X) | SWIZZLE_COMP(1, COMP_Y) |                         \
   SWIZZLE_COMP(2, COMP_Z) | SWIZZLE_COMP(3, COMP_W))
...
struct Uniform {
  ...
  inline bool operator<(const Uniform &rhs) const { return pos < rhs.pos; }
...
#define MAX_OUTPUT 16
  ...
  int maxOutputReg() const { return isGeoShader ? 0x07 : 0x10; }

  DVLEData(const char *filename)
      : filename(filename), entrypoint("main"), nodvle(false),
        isGeoShader(false), isCompatGeoShader(false), isMerge(false),
        inputMask(0), outputMask(0), geoShaderType(0), geoShaderFixedStart(0),
        geoShaderVariableNum(0), geoShaderFixedNum(0), uniformCount(0),
        symbolSize(0), constantCount(0), outputUsedReg(0), outputCount(0) {}
};