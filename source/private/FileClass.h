...
public:
  FileClass(const char *file, const char *mode)
      : LittleEndian(true), own(true), filePos(0) {
    // Do nothing
  }
  ~FileClass() {
    // Do nothing
  ...
  void WriteByte(byte_t value) { _RawWrite(&value, sizeof(byte_t)); }
  ...
  bool ReadRaw(void *buffer, size_t size) {
    return _RawRead(buffer, size) == size;
  }
  bool WriteRaw(const void *buffer, size_t size) {
    return _RawWrite(buffer, size) == size;
  }
